---
title: Why Thai Text on the Web or E-Readers Breaks Characters Wrong
description: >-
...
Both screenshots came from real devices, real situations, not a simulation. And critically: **opening the same page in Chrome on my own laptop showed no problem at all.** The text wrapped perfectly normally.

</div>

That gap, same page, same content, completely different rendering across machines, is where the whole investigation started.

## Wrong hypothesis #1: "WebKit doesn't support Thai word segmentation"
...
| **`word-break: keep-all` + `overflow-wrap: break-word`** (what this site actually had) | **Breaks mid-word: the only one that fails, out of five** |

## The real cause: a bug we shipped ourselves
...
Sometimes "the browser is broken" is actually "our own multi-year-old CSS is broken." Don't trust the first plausible hypothesis; test against a real reference site before concluding anything.

</div>

## What's left: real words the browser's own dictionary still splits wrong
...
- **เจ้าหน้าที่**, **สถานทูต**, **พาสปอร์ต**, **ต้นสังกัด**, **เร่งด่วน**, **ใบเสร็จ**, **สถานกงสุล**, words that repeat constantly throughout the article, adding up to **253 mis-split occurrences**, not the 2-3 I'd spotted just by eyeballing screenshots.

That reframes the problem: "fix the CSS and you're done" becomes "now we need a systematic way to find every word the browser's dictionary gets wrong."

## Shopping for "ground truth": except tokenizers don't agree with each other
...
## What about other scripts that write without spaces too?
...
Everything below is implemented and running, not just planned:

1. **Deleted `word-break: keep-all` from the CSS**, handling almost all of the problem, at zero cost.
2. **A 70-word list of words the browser still gets wrong**, kept in `scripts/thai-word-joins.json`, generated using `nlpo3-newmm-typescript` as the candidate source (faster than `wordcut`, no สหรัฐฯ bug), diffed against `Intl.Segmenter`, and **hand-filtered** every time, dropping ordinary phrases like "เพื่อให้" and keeping only genuinely atomic units like "ตัวเอง".
3. **Processed at build time, not runtime**: `scripts/apply-thai-word-joins.mjs` runs as a `postbuild` step after every `astro build`, rewriting the already-built HTML directly, inserting zero-width joiners (U+2060) around listed words, automatically skipping code, links, and headings (so anchor IDs stay intact), instead of making every reader run word-breaking logic on every page load. Running it for real against this article's own build produced exactly the number I'd estimated earlier: **253 fixed spots**.
4. **No tokenizer or dictionary ever ships to a reader**: `nlpo3-newmm-typescript` and `cheerio` live in `devDependencies` only. What readers get is still plain static HTML, not a single extra byte of runtime weight.
5. **A dev script for auditing new Thai posts**: `npm run thai:audit` scans freshly built content for new candidate words (never auto-merging them into the approved list, for the reason explained above, running it against this very article surfaced 346 new candidates, most of which were ordinary phrases that shouldn't be added).
...
<button type="button" class="thai-demo-tab" data-mode="broken">Simulated: no boundary info at all</button>
<button type="button" class="thai-demo-tab" data-mode="native">Browser default (after the CSS fix)</button>
<button type="button" class="thai-demo-tab" data-mode="fixed">With word-joiner list (live state)</button>
